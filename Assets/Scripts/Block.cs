...
public class Block : MonoBehaviour
{
    ...
    private void OnTriggerEnter2D(Collider2D collision)
    {
        if (collision != null && collision.gameObject.tag == gameObject.tag)
        {
            ...
            if (ID < collision.gameObject.GetInstanceID()) return;
            GetComponent<Animator>().SetTrigger("Scale");
            collision.gameObject.GetComponent<Animator>().SetTrigger("Scale");
            GameManager.Instance.levels[GameManager.Instance.GetCurrentIndex()].SpawnObj();
